...
| Q3 Observability | 5 | **4** | Traces live on EU project `cmthhhzzv02wsad0d4qogeznv`: two 4 Sep Slice 1 runs, named `Requirement Extractor` generation nested under a `prd-genie-slice1` root, tokens 240/395/635, cost $0.00455, I/O inspectable. Wiring (HTTP ingest, Option C) named in README. Screenshot [langfuse-traces.png](../evidence/screenshots/langfuse-traces.png). Score configs verified 5 Sep (`GET /api/public/score-configs`): `Completeness`, `Hallucination`, `Groundedness` — all NUMERIC 0–1 (Title Case names). Detail in [baseline-results.md](../evidence/baseline-results.md) §Observability proof run | **−1, not 5.** Criterion 6 (configs exist) is met. Remaining minus: no graded T1 row (T1 stays `Not run`; 4 Sep input was the long transcript), `GET /api/public/scores` is empty so no generation carries a score yet, and only Hallucination has an LLM-as-judge evaluator (Completeness / Groundedness are configs only). Per-agent multi-span is unproven because only one agent exists. |
...
1. Re-run Extractor on the **short T1 string** until **T1 green** in baseline-results (the 4 Sep long-transcript run does not count). That also gives a generation the Hallucination evaluator can score (it will not backfill 4 Sep) → **+1** observability (4→5) and the start of **+12** core (full 12 only when extract + PRD + stories e2e). Configs exist; the observability minus is the missing graded T1 / empty scores list, not missing Langfuse wiring.
