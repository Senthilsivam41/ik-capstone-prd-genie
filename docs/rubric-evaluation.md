...
| Q3 Observability | 5 | **4** | Traces live on EU project `cmthhhzzv02wsad0d4qogeznv`: two 4 Sep Slice 1 runs, named `Requirement Extractor` generation nested under a `prd-genie-slice1` root, tokens 240/395/635, cost $0.00455, I/O inspectable. Wiring (HTTP ingest, Option C) named in README. Screenshot [langfuse-traces.png](../evidence/screenshots/langfuse-traces.png). Score configs and matching evaluators verified 5 Sep: `Completeness`, `Hallucination`, `Groundedness` — all NUMERIC 0–1, assigned at sampling 1. Detail in [baseline-results.md](../evidence/baseline-results.md) §Observability proof run | **−1, not 5.** Remaining minus: no graded T1 row (T1 stays `Not run`; 4 Sep input was the long transcript) and `GET /api/public/scores` is empty until that run. |
...
1. Re-run Extractor on the **short T1 string** until **T1 green** in baseline-results (the 4 Sep long-transcript run does not count). That generation can receive Completeness, Hallucination and Groundedness scores at sampling 1 (no backfill of 4 Sep) → **+1** observability (4→5) and the start of **+12** core.
