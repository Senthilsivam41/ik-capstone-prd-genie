...
- n8n Manual Trigger + text input on the IK instance can run all 12 baseline inputs without custom scripting.
- Langfuse EU free/hobby tier is sufficient for trace volume across a 2-week build (`cloud.langfuse.com` project `cmthhhzzv02wsad0d4qogeznv`).
...
|---|---|---|---|
| n8n IK Cloud (`agenticai100.app.n8n.cloud`) | Sendil | Signed in — OpenAI credential present | Low |
| Langfuse EU project `my-capstone-prd-genie` (`cmthhhzzv02wsad0d4qogeznv`) | Sendil | **Traces proven** — Extractor generation with tokens + cost, 4 Sep | Low |
| Model API access (OpenAI via IK keys in n8n) | Sendil | Connection tested in n8n | Low |
...
| **Document Ingestion** | n8n Manual Trigger / text | All provided inputs are plain `.txt`/`.md`; no PDF OCR or CSV reshaping needed |
