...
| E1b | 2026-09-05 | Extractor rule 9 only: Constraints section is mandatory when capacity / p95 / API version is stated; each row `class: NFR`. | T7 Fail `87076a54` (no Constraints) | *pending — re-run T7 after n8n prompt paste* | pending | Keep only if T7 output has `class: NFR` and exact numbers |
| E3 | | | | | | |
